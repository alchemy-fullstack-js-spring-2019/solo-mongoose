...
describe('testing routes', () => {
  ...
  describe('Ship routes', () => {
    it('creates a new Ship document with POST', () => {
      return Pirate.create({ name: 'Captain Hook', ship: 'The Ticking Tock' })
        .then(createdPirate => {
          return request(app)
            .post('/ships')
            .send({ 
              name: createdPirate.ship,
              owner: createdPirate._id,
              sailCount: 2, color: 'Not Black' 
            })
            .then(res => {
              expect(res.body).toEqual({
                name: 'The Ticking Tock',
                owner: createdPirate._id,
                sailCount: 2,
                color: 'Not Black',
